...
import { submitResult, resolvePrediction } from "../services/api";
...
export default function HistoryPage() {
  ...
  const groupHistory = useMemo(() => {
    ...
    for (const pred of data) {
      ...
      existing.games += 1;
      existing.highRisk =
        existing.highRisk || !!pred.prediction_group_is_high_risk;
      ...
      const isCorrect =
        isResolved && resolved.actual_outcome === pred.predicted_outcome;
    ...
    return Array.from(groups.values())
      .map((g) => {
        ...
        const groupIndex = g.groupIndex != null ? Number(g.groupIndex) : null;
        return {
          ...g,
          isResolved,
          status: isResolved
          ...
          groupIndex,
          groupLabel:
            groupIndex != null && !Number.isNaN(groupIndex)
              ? `G${groupIndex}`
              : "GROUP",
        };
      })
      .sort((a, b) => {
        const aIndex =
          a.groupIndex != null && !Number.isNaN(a.groupIndex)
            ? a.groupIndex
            : Number.POSITIVE_INFINITY;
        const bIndex =
          b.groupIndex != null && !Number.isNaN(b.groupIndex)
            ? b.groupIndex
            : Number.POSITIVE_INFINITY;
  ...
  const historySummary = useMemo(() => {
    ...
    const resolved = Object.keys(resolvedMatches).length;
    const grouped = groupHistory.length;
    const hits = groupHistory.reduce(
      (sum, group) => sum + (group.hits || 0),
      0,
    );
    const groupedResolved = groupHistory.filter(
      (group) => group.isResolved,
    ).length;
    return {
      total,
      resolved,
  ...
  }, [data.length, resolvedMatches, groupHistory]);
  ...
  const predictionsByDate = useMemo(() => {
    ...
    for (const pred of filtered) {
      const key =
        pred.match_date || pred.timestamp?.slice?.(0, 10) || "Unknown";
  ...
  const handleResultSubmit = async (e) => {
    ...
    if (!matchId.trim()) {
      setSubmitMsg({
        type: "error",
        text: "Match ID is required — click a row to copy it.",
      });
      return;
    }
    ...
    if (isNaN(hs) || isNaN(as_) || hs < 0 || as_ < 0) {
      setSubmitMsg({
        type: "error",
        text: "Scores must be non-negative integers.",
      });
      return;
    }
    ...
    try {
      ...
      setSubmitMsg({
        type: "success",
        text: `Result recorded: ${outcome.replace("_", " ").toUpperCase()}`,
      });
      setResultForm({
        matchId: "",
        homeScore: "",
  ...
  const handleResolveRequest = async (prediction) => {
    if (!prediction) return;

    setResolvingMatchId(prediction.match_id || "");
    setResolveMsg({
      type: "info",
      text: `Resolving ${prediction.home_team} vs ${prediction.away_team}...`,
    });

    try {
      const res = await resolvePrediction(prediction.match_id);
      setResolveMsg({
        type: "success",
        text:
          res.data?.status === "resolved"
            ? `Resolved ${prediction.home_team} vs ${prediction.away_team}`
            : res.data?.status === "already_resolved"
              ? "That match is already resolved."
              : res.data?.message || "Resolution complete.",
      });
      ...
      setResolveMsg({
        type: "error",
        text: err.response?.data?.detail || "Resolution trigger failed",
      });
    } finally {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="space-y-4 mb-5">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div className="card p-4">
            ...
            <p className="font-display text-2xl text-brand-greenlight tabular-nums">
              {historySummary.resolved}
            </p>
          </div>
          <div className="card p-4">
            <p className="label mb-2">GROUPS</p>
            <p className="font-display text-2xl text-white tabular-nums">
              {historySummary.grouped}
            </p>
          </div>
          <div className="card p-4">
            <p className="label mb-2">HIT RATE</p>
            <p className="font-display text-2xl text-white tabular-nums">
              {Math.round(historySummary.hitRate * 100)}%
            </p>
          </div>
        </div>
      ...
      {resolveMsg && (
        <div
          className={`font-display text-xs px-4 py-3 rounded-sm border mb-4 ${
            resolveMsg.type === "success"
              ? "text-brand-greenlight bg-brand-greendark border-brand-green"
              : resolveMsg.type === "error"
                ? "text-brand-redlight bg-brand-reddark border-brand-red"
                : "text-gray-300 bg-brand-darkgray border-brand-midgray"
          }`}
        >
          {resolveMsg.text}
        </div>
      ...
      {view === "table" ? (
        ...
        <div className="space-y-5">
          ...
          ) : predictionsByDate.length ? (
            predictionsByDate.map(({ date, items }) => (
                          ...
                          <td colSpan={6} className="px-4 py-3">
                            <div className="space-y-2">
                              {g.gameItems.map((item) => (
                                <div
                                  key={item.matchId}
                                  className="flex flex-wrap items-center justify-between gap-2 bg-brand-gray/40 border border-brand-midgray rounded-sm px-3 py-2"
                                >
                                  <div>
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span
                                      className={`font-display text-[10px] px-2 py-0.5 rounded-sm border ${item.isResolved ? (item.isCorrect ? "text-brand-greenlight bg-brand-greendark border-brand-green" : "text-brand-redlight bg-brand-reddark border-brand-red") : "text-gray-400 border-brand-midgray"}`}
                                    >
                                      {item.isResolved
                                        ? item.isCorrect
                                          ? "CORRECT"
      ...
      <div ref={resultFormRef} className="mt-8 card p-5">
        <p className="label mb-4">
          SUBMIT ACTUAL RESULT — TRIGGERS LEARNING UPDATE
        </p>
        <form
        ...
        >
          ...
          <div className="md:col-span-2">
            ...
            <input
              ...
              onChange={(e) =>
                setResultForm((p) => ({ ...p, matchId: e.target.value }))
              }
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setResultForm((p) => ({ ...p, homeScore: e.target.value }))
              }
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setResultForm((p) => ({ ...p, awayScore: e.target.value }))
              }
              className="w-full bg-brand-darkgray border border-brand-midgray focus:border-brand-red outline-none text-white font-display text-xs px-3 py-2 rounded-sm"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setResultForm((p) => ({ ...p, date: e.target.value }))
              }
              className="w-full bg-brand-darkgray border border-brand-midgray focus:border-brand-red outline-none text-white font-display text-xs px-3 py-2 rounded-sm"
            />
          </div>
        ...
        {submitMsg && (
          <div
            className={`mt-3 font-display text-xs px-3 py-2 rounded-sm ${
