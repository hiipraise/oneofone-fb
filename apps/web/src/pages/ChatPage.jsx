  ...
  return (
    ...
    >
      <div className={`max-w-[82%] ${isUser ? "order-2" : "order-1"}`}>
        ...
        >
          <span
            className={`font-display text-xs ${isUser ? "text-gray-600" : "text-brand-red"}`}
          >
            {isUser ? "YOU" : "1/1 AI"}
          </span>
          {msg.timestamp && (
        ...
        {normalizedSources.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1">
            {normalizedSources.map((src, i) => {
              const safeLink = src.link.startsWith("http")
                ? src.link
                : `https://${src.link}`;
              return (
                <div
...
function SessionBar({ summary, sessionId, onNew, onHistory, onDelete }) {
  ...
  const teams = [
    ...new Set((summary.teams_discussed || []).filter(Boolean)),
  ].slice(0, 4);
  ...
  const loadSession = async (sid) => {
    ...
    try {
      const [histRes, sumRes] = await Promise.all([
        ...
        import("../services/api")
          .then((m) => m.getSessionSummary(sid))
          .catch(() => ({ data: null })),
      ]);
        ...
        setMessages([
          {
            role: "assistant",
            content:
              "Session loaded. No messages yet — ask me anything about Football or Soccer.",
    ...
    <div className="flex flex-col h-full max-h-[calc(100vh-120px)] animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-3 shrink-0">
        <div>
          ...
          <p className="font-body text-xs text-gray-600 mt-0.5">
            Groq LLM · live web search · persistent memory ·{" "}
      ...
      {showSuggestions && !loading && messages.length <= 1 && (
        <div className="shrink-0 mb-2">
          <Suggestions sport={sport || "soccer"} onPick={handleSuggestion} />
        </div>
      )}
      ...
      {input.length > 0 && (
        <p className="font-display text-xs text-gray-700 mt-1 text-right">
          {input.length}/{chatMessageMax} · Enter to send · Shift+Enter for new
          line
        </p>
      )}
    </div>
