...
class PredictionEngine:
    ...
    def predict(self, features: Dict[str, float], sport: str = "soccer") -> Dict[str, Any]:
        ...
        if self.is_trained.get(sport) and model is not None and w_ml > 0:
            try:
                expected_features = int(getattr(self.scalers[sport], "n_features_in_", fv.shape[0]))
                assert fv.shape[0] == expected_features, (
                    f"Feature width mismatch: got {fv.shape[0]} expected {expected_features}. "
                    "Retrain model artifacts for the current feature set."
                )
